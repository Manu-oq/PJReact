...
describe("CertificacionesPage", () => {
  ...
  it("renderiza estado inicial y carga tipos, casos, preview, warnings y errores", async () => {
    ...
    expect(serviceMocks.getCertificationTypes).toHaveBeenCalledTimes(1);
    expect(serviceMocks.getCertificationCases).toHaveBeenCalledTimes(1);
  ...
  it("permite crear un caso con PDFs y lo selecciona automáticamente", async () => {
    ...
    fireEvent.change(container.querySelector('input[aria-label="Primera instancia PDF"]'), {
      target: { files: [fileOne] },
    });
    fireEvent.change(container.querySelector('input[aria-label="Segunda instancia PDF"]'), {
    ...
    await waitFor(() => {
      expect(serviceMocks.createCertificationCase).toHaveBeenCalledWith({
        type_code: "resumen",
        title: "Caso nuevo",
        primera_instancia: fileOne,
        segunda_instancia: fileTwo,
      });
    });

    expect(await screen.findByText(/Caso creado correctamente/i)).toBeInTheDocument();
    expect(screen.getAllByText(/Caso #2/i).length).toBeGreaterThan(0);
    expect(screen.getByText(/Preview nuevo/i)).toBeInTheDocument();
  });

  it("permite seleccionar un caso y guardar overrides básicos", async () => {
    const caseOne = buildCase();
    const caseTwo = buildCase({
      id: 2,
      title: "Caso seleccionado",
      resolved_fields: { nombre: "María" },
      missing_fields: { rit: "Falta RIT" },
      validation_errors: {},
      warnings: [],
      document_preview: "Preview caso 2",
    });
    const extractionOne = buildExtraction();
    const extractionTwo = buildExtraction({
      case_id: 2,
      resolved_fields: { nombre: "María" },
      missing_fields: { rit: "Falta RIT" },
    ...
    const updatedCaseTwo = {
      ...caseTwo,
      resolved_fields: { nombre: "María", rit: "123-2026" },
      missing_fields: {},
    };
    const updatedExtractionTwo = {
      ...extractionTwo,
      manual_overrides: { rit: "123-2026" },
      resolved_fields: { nombre: "María", rit: "123-2026" },
      missing_fields: {},
    };
    ...
    serviceMocks.getCertificationTypes.mockResolvedValue([buildType()]);
    serviceMocks.getCertificationCases
      .mockResolvedValueOnce([caseOne, caseTwo])
      .mockResolvedValueOnce([caseOne, updatedCaseTwo]);
    serviceMocks.getCertificationCase
      .mockResolvedValueOnce(caseOne)
      .mockResolvedValueOnce(caseTwo)
      .mockResolvedValueOnce(updatedCaseTwo);
    serviceMocks.getCertificationExtraction
      .mockResolvedValueOnce(extractionOne)
      .mockResolvedValueOnce(extractionTwo)
      .mockResolvedValueOnce(updatedExtractionTwo);
    serviceMocks.updateCertificationExtraction.mockResolvedValue(updatedExtractionTwo);

    render(<CertificacionesPage />);

    expect((await screen.findAllByText(/Caso #1/i)).length).toBeGreaterThan(0);

    fireEvent.click(screen.getAllByText(/Caso #2/i)[0]);

    expect(await screen.findByText(/Preview caso 2/i)).toBeInTheDocument();

    fireEvent.change(screen.getByLabelText("RIT"), {
      target: { value: "123-2026" },
    });
    fireEvent.click(screen.getByRole("button", { name: /Guardar correcciones/i }));

    await waitFor(() => {
      expect(serviceMocks.updateCertificationExtraction).toHaveBeenCalledWith(2, {
        nombre: "María",
        rit: "123-2026",
      });
    });

    expect(await screen.findByText(/Correcciones guardadas correctamente/i)).toBeInTheDocument();
  });

  it("permite validar, generar y descargar el documento", async () => {
